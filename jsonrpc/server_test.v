...
fn test_server() ? {
	...
	hello_result := client.send<string, RpcResult<string>>('hello', '') or { return err }
	...
	client.send<[]string, RpcResult<string>>('mirror', ['0']) or { assert err is none }
}
...
fn test_interceptor() ? {
	...
	client.send<SumParams, RpcResult<int>>('sum', SumParams{ nums: [1, 2, 4] }) or { return err }
	assert test_inter.methods_recv.len == 1
	assert test_inter.methods_recv[0] == 'sum'
	assert test_inter.messages.len == 1

	client.send<string, RpcResult<string>>('trigger', '') or { return err }
