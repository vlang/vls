...
fn test_set_features() {
	...
	assert ls.features() == [.diagnostics, .document_symbol, .workspace_symbol, .signature_help, .completion, .hover, .folding_range, .definition]
	ls.set_features(['formatting'], true) or {
		assert false
		return
	}
	assert ls.features() == [.diagnostics, .document_symbol, .workspace_symbol, .signature_help, .completion, .hover, .folding_range, .definition, .formatting]
